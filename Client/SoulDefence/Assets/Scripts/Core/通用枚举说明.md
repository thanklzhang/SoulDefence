...
// Buff属性修改
AttributeType attrType = AttributeType.AttackPower;
float bonusValue = 50f;

// 被动技能属性加成
passiveSkill.attributeType = AttributeType.Defense;
passiveSkill.attributeBonusValue = 20f;
...
| AttributeType | 属性类型 | AttackPower~DamageReduction (0~8) | Buff、被动技能、属性加成 |
...
```csharp
// 旧代码（使用int）
public int attributeType = 0;  // 0=攻击力, 1=防御力...

// 新代码（使用枚举）
public AttributeType attributeType = AttributeType.AttackPower;
```
